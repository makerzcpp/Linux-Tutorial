...
- Redis 安装
    ...
    - Redis 下载：`wget http://download.redis.io/releases/redis-3.0.7.tar.gz` （大小：1.4 M）
        ...
        - 开放防火墙端口：
            - 添加规则：`firewall-cmd --zone=public --add-port=6379/tcp --permanent`<br>
            - 重新载入：`firewall-cmd --reload`<br>
            - 查看：`firewall-cmd --zone=public--query-port=6379/tcp`<br>
	    - 删除：`firewall-cmd --zone=public==remove-port=6379/tcp --permanent`<br>
